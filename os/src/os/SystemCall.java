...
public class SystemCall {
    ...
    // Track files created by writeFile so reset can safely clean simulation artifacts
    ...
    public static int writeFile(String filename, String data) {
        ...
        try {
            ...
            try {
                generatedFiles.add(new File(filename).getCanonicalPath());
            } catch (IOException e) {
                generatedFiles.add(new File(filename).getAbsolutePath());
            }
    ...
    /**
     * Delete files generated by writeFile in prior simulation runs.
     * Preserves bundled sample programs Program1/2/3.txt.
     *
     * @return number of files deleted successfully
     */
    public static int cleanupGeneratedFiles() {
        int deletedCount = 0;
        Set<String> preservedNames = new HashSet<>(Arrays.asList("Program1.txt", "Program2.txt", "Program3.txt"));
        ...
        while (iterator.hasNext()) {
            ...
            if (preservedNames.contains(file.getName())) {
                iterator.remove();
                continue;
            }
